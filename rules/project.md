...
"빈 도화지" 같은 CMS를 만든다.
초기 상태에서는 아무런 콘텐츠 폼이나 페이지가 정의되어 있지 않으며, 사용자가 자신의 목적에 맞게 자유롭게 구조를 설계하고 커스터마이징할 수 있는 CMS 시스템을 개발한다.

### 제품 전략 (에디션 & 개발 로드맵)

개발 순서: **1단계 → 2단계 → 3단계**

#### 1단계 — 완성형 웹사이트 빌더 (현재 개발 중)

CMS + Admin Panel + User Site + Page Builder로 구성되는 핵심 제품.
1단계 내에서 두 가지 에디션으로 나뉜다:
...
**Starter 에디션 기능:**
- 콘텐츠 폼 카테고리(FormCategory)/콘텐츠 폼(ContentForm)/콘텐츠 CRUD, 미디어 관리(폴더 포함), 역할/권한, 회원/관리자 관리
- 버전 관리, 예약 발행, 콘텐츠 보관(Archive), 콘텐츠 상세 보기(readOnly)
- REST API (Swagger + API 가이드 페이지 83개 엔드포인트 문서화)
- Public API + API 키 인증 (외부 프론트엔드 연동)
- 회원 인증 (가입/로그인/이메일 인증/비밀번호 재설정)
- 검색 (MeiliSearch, DB 폴백), 알림 (WebSocket 실시간, 역할 기반 필터링, 헤더 벨 아이콘)
- 이미지 최적화 (Sharp 썸네일/WebP), 감사 로그, Webhook (Slack/Discord 자동 형식 변환)
- 백업/복원 (DB + 업로드 파일 ZIP 통합), Import/Export (CSV/JSON, 덮어쓰기 옵션)
- 휴지통 (소프트 삭제 + 30일 자동 정리, 별도 UI 없음)
- 시스템 설정 (사이트 정보/로고/파비콘/SMTP 이메일/타임존/유지보수 모드)
- 대시보드 통계, 권한 기반 메뉴/접근/버튼 제어 (PermissionGuard)
...
1단계(Business) 위에 **조직 관리 및 엔터프라이즈 기능**을 추가한다.
...
| **GraphQL API** | REST에 더해 유연한 쿼리 인터페이스 제공 |
| **감사 로그 대시보드 강화** | 관리자 활동 추적, 필터링, CSV 내보내기 강화 |
...
1단계 또는 2단계를 기반으로 **특정 산업에 특화된 솔루션**을 제공한다.
1단계 완성 후 가장 시장성이 큰 업종 하나를 선택하여 깊이 있게 개발한다.
...
- **사용자 정의**: 콘텐츠 폼, 필드, 관계, 페이지 구조를 사용자가 직접 설계
- **콘텐츠 폼 카테고리(FormCategory)**: 콘텐츠 폼을 그룹핑하여 사이드바 서브그룹으로 정렬(order 기준)
...
- **다국어(i18n) 지원**: 콘텐츠를 여러 언어로 관리, 언어별 버전 관리 `[Business+]`
- **버전 관리 / 히스토리**: 콘텐츠 수정 이력 저장, 이전 버전으로 롤백 가능
- **예약 발행**: 특정 날짜/시간에 콘텐츠 자동 발행, 발행 종료일 설정
- **REST API**: 콘텐츠 폼 정의 시 REST API 자동 생성, Swagger 문서 자동 생성
- **GraphQL API**: REST에 더해 유연한 쿼리 인터페이스 제공 `[Enterprise]`
- **알림 센터**: 콘텐츠별 알림 대상 설정, 관리자 알림 통합 관리
- **이미지 최적화**: 자동 리사이즈, WebP 변환, 썸네일 생성, lazy loading
- **SEO 관리**: 메타 태그, OG 태그, sitemap.xml, URL 슬러그 등 검색 엔진 최적화 `[Business+]`
...
```
Admin Panel (Starter — 현재 구현 완료)
├── 대시보드 (통계 카드 + 최근 활동)
├── 콘텐츠 폼 카테고리 (1뎁스, FormCategory CRUD)
├── 콘텐츠 폼 (1뎁스, ContentForm CRUD — 이전 명칭: 콘텐츠 타입)
├── 파일 관리 (1뎁스)
├── 콘텐츠 (2뎁스 그룹 — 콘텐츠 폼 목록을 카테고리별 서브그룹으로 렌더링)
│   ├── [카테고리 A]
│   │   ├── (게시글)
│   │   └── (뉴스)
│   ├── [카테고리 B]
...
│   ├── 사용자 관리
│   └── 역할/권한
├── 외부 연동 (2뎁스 그룹, 슈퍼관리자/관리자 전용)
│   ├── API 가이드
│   ├── API 키 관리 (슈퍼관리자 전용)
│   └── Webhook
├── 운영 / 데이터 (2뎁스 그룹, 슈퍼관리자/관리자 전용)
│   ├── 감사 로그
│   ├── Import/Export
│   └── 백업/복원 (슈퍼관리자 전용)
└── 시스템 설정 (관리자 이상)

메뉴는 사용자 역할/권한에 따라 동적으로 표시/숨김 (hasPermission + PermissionGuard)
2뎁스 그룹은 아코디언 방식 (한 번에 하나만 펼침, grid 애니메이션)

Admin Panel (Business — 향후 추가 예정)
├── 페이지 관리 [Business+]
├── 템플릿 관리 [Business+]
├── 컴포넌트 관리 [Business+]
├── SEO 설정 [Business+]
├── 다국어(i18n) 설정 [Business+]

Admin Panel (Enterprise — 향후 추가 예정)
├── 워크플로우 [Enterprise]
├── 내부 댓글/메모 [Enterprise]
```
...
| 하위 메뉴 | 설명 |
|-----------|------|
| 콘텐츠 폼 카테고리 | 콘텐츠 폼을 그룹핑하기 위한 카테고리(FormCategory) CRUD. 사이드바 "콘텐츠" 그룹의 서브그룹 정렬에 사용 (order 기반) |
| 콘텐츠 폼 정의 | "게시글", "상품", "FAQ" 같은 콘텐츠 폼 생성 (이전 명칭: 콘텐츠 타입). 각 폼에 필드 추가 (제목, 내용, 가격, 이미지 등). 카테고리 지정 가능. 필드 타입: 텍스트, 숫자, 날짜, 이미지, 관계 등 |
| 콘텐츠 목록/CRUD | 정의된 콘텐츠 폼별로 실제 콘텐츠 작성/수정/삭제. 예: "게시글" 폼 → 실제 게시글 작성 |
...
| 역할/권한 설정 | 역할 생성 (슈퍼관리자, 편집자, 뷰어 등). 각 역할에 권한 할당 (어떤 메뉴에 접근 가능한지). 콘텐츠 폼 카테고리 권한은 별도 신설 없이 `content-form:create/update/delete`와 1:1 매핑 |
...
- 콘텐츠 폼별 알림 대상 설정 (어떤 역할/관리자에게 알릴지)
...
| 컴포넌트 | 설명 |
|----------|------|
| 콘텐츠 목록 | 특정 콘텐츠 폼의 목록을 표시 (게시판 목록, 상품 목록 등) |
...
| 페이지 타입 | 설명 |
|-------------|------|
| 게시판 | 목록 + 상세 + 작성 페이지가 세트로 제공 (콘텐츠 폼 연결) |
| 갤러리 | 이미지/동영상 갤러리 페이지 |
| 검색 결과 | 콘텐츠 검색 결과 페이지 |
...
| 입력 | 텍스트 에디터, 파일 업로더, 폼 |
| 인터랙티브 | 버튼, 링크, 팝업 |
| 동적 | 콘텐츠 목록, 콘텐츠 상세, 콘텐츠 작성폼 (콘텐츠 폼 연결) |
...
| 영역 | 권한 예시 |
|------|-----------|
| **Admin Panel** | 콘텐츠 CRUD, 콘텐츠 폼 정의(`content-form:*`), 콘텐츠 폼 카테고리(콘텐츠 폼 권한과 1:1 매핑), 회원 관리, 관리자 관리, 미디어 관리, 시스템 설정 |
...
콘텐츠 폼별로 어떤 관리자/역할에게 알림을 보낼지 설정할 수 있다.
...
콘텐츠 폼 정의 시 API가 자동 생성된다.

| 기능 | 설명 |
|------|------|
| **REST API** | 콘텐츠 폼별 CRUD 엔드포인트 자동 생성 `[Starter+]` |
| **GraphQL API** | 유연한 쿼리를 위한 GraphQL 지원 `[Enterprise]` |
| **Swagger 문서** | API 문서 자동 생성 `[Starter+]` |
| **API 키 관리** | 클라이언트별 API 키 발급/관리 `[Starter+]` |
| **Rate Limiting** | API 요청 제한 (남용 방지) `[Starter+]` |
...
- 1단계 기능을 우선 완성한 후, 2단계(Enterprise) 기능을 추가 개발
...
- 에디션에 따라 Admin Panel의 메뉴/라우트를 동적으로 제어
  - Starter: CMS 핵심 메뉴만 표시 (콘텐츠 폼 카테고리, 콘텐츠 폼, 콘텐츠, 미디어, 회원/역할 등)
