...
1. 시스템 설정 → 사이트 기본 정보 입력
   - 사이트 이름, 로고, 파비콘 설정
   - 기본 언어, 타임존 설정 (※ 단일 언어 설정은 Starter, 다국어 콘텐츠 관리는 [Business+])
...
```
1. 콘텐츠 관리 → 콘텐츠 폼 카테고리 → "공지" 카테고리 생성 (선택)
   - 이름: "공지", 슬러그: "notice-group", order: 0

2. 콘텐츠 관리 → 콘텐츠 폼 정의 → 새 콘텐츠 폼 생성
   - 이름: "공지사항"
   - 슬러그: "notice"
   - 카테고리: "공지" (드롭다운에서 선택, 미지정 가능)
...
4. 페이지 관리 → 동적 페이지 → 새 페이지 생성
   - 페이지 타입: 게시판
   - 연결 콘텐츠 폼: "공지사항"
   - URL: /notice

5. 공통 레이아웃 → 네비게이션에 메뉴 추가
   - 메뉴명: "공지사항"
   - 링크: /notice
...
3. 저장 (초안) 또는 발행
   - 워크플로우 활성화 시: 검토 요청 [Enterprise]
   - 워크플로우 비활성화 시: 즉시 발행
```

### 2.3 콘텐츠 승인 (워크플로우) `[Enterprise]`
...
4. 각 컴포넌트 설정
   - 히어로 배너: 이미지, 텍스트, 링크
   - 최신 공지: 콘텐츠 폼 연결, 표시 개수
   - 인기 상품: 콘텐츠 폼 연결, 정렬 기준
...
2. REST API 호출 (Public API)
   GET /public/content-forms               # 콘텐츠 폼 목록
   GET /public/contents/notice             # "notice" 폼의 콘텐츠 목록
