...
- **완전한 자유도**: 콘텐츠 폼, 필드, 관계, 페이지 구조를 사용자가 직접 정의
...
- **고객 서버(또는 호스팅)에 CMS를 설치**하고, 고객 요구사항에 맞게 콘텐츠 폼/페이지/디자인을 세팅해서 납품
...
┌──────────────────────────────────────────────┐
│              Admin Panel (관리자)            │
│  콘텐츠 폼/콘텐츠, 회원/역할, 미디어, 알림(WebSocket) │
│  검색, 백업/복원, Import/Export, API 가이드        │
└─────────────────────┬────────────────────────┘
                      │ REST / GraphQL API
┌─────────────────────┴────────────────────────┐
│              NestJS Backend (서버)            │
...
├── 대시보드                           ← 전체 현황
├── 콘텐츠 관리
│   ├── 콘텐츠 폼 카테고리             ← 콘텐츠 폼을 분류하기 위한 그룹(FormCategory)
│   ├── 콘텐츠 폼 정의                 ← "게시글", "상품" 등 스키마 생성 (이전 명칭: 콘텐츠 타입)
...
| 기능                    | 설명                                               | 에디션       |
| ----------------------- | -------------------------------------------------- | ------------ |
| 콘텐츠 폼/콘텐츠 CRUD   | 사용자 정의 스키마로 동적 콘텐츠 관리              | Starter      |
| 콘텐츠 폼 카테고리      | FormCategory로 콘텐츠 폼을 그룹핑, 사이드바 서브그룹 렌더링 | Starter |
| 미디어 관리             | 파일 업로드, 폴더 정리, 이미지 최적화              | Starter      |
| 역할/권한               | Role 기반 세밀한 권한 관리                         | Starter      |
| 버전 관리               | 수정 이력 저장, 롤백                               | Starter      |
| 예약 발행               | 특정 시간 자동 발행/종료                           | Starter      |
| 콘텐츠 보관             | Archive/Unarchive 상태 관리                        | Starter      |
| API 자동 생성           | REST API + Swagger + API 가이드 페이지             | Starter      |
| 검색                    | MeiliSearch 전문 검색 (DB 폴백 지원)               | Starter      |
| 이미지 최적화           | 자동 리사이즈, WebP 변환, 썸네일                   | Starter      |
| 알림 센터               | WebSocket 실시간, 역할 기반 필터링                 | Starter      |
| 감사 로그               | 자동 활동 기록, 필터/검색/페이지네이션              | Starter      |
| Webhook                 | 이벤트별 외부 서비스 연동, Slack/Discord 호환       | Starter      |
| 백업/복원               | DB + 업로드 파일 ZIP 통합 백업, 파일 업로드 복원    | Starter      |
| 다국어 (i18n)           | 콘텐츠 다국어 버전 관리                            | Business     |
| SEO 관리                | 메타태그, OG, sitemap, 슬러그                      | Business     |
...
| **FormCategory**| 5개        | 콘텐츠 폼 카테고리 CRUD (콘텐츠 폼 그룹핑)  | ✅ 완료 |
| **ContentForm** | 5개        | 동적 콘텐츠 폼 정의 CRUD                    | ✅ 완료 |
...
**FormCategory (5개)**
```
POST   /form-categories                콘텐츠 폼 카테고리 생성
GET    /form-categories                카테고리 목록 (order 정렬)
GET    /form-categories/:id            카테고리 상세
PATCH  /form-categories/:id            카테고리 수정
DELETE /form-categories/:id            카테고리 삭제 (소속 콘텐츠 폼의 categoryId는 SetNull)
```

**ContentForm (5개)**
```
POST   /content-forms                  콘텐츠 폼 생성 (필드 정의 + categoryId 포함)
GET    /content-forms                  콘텐츠 폼 목록
GET    /content-forms/:id              콘텐츠 폼 상세 (ID 또는 slug)
PATCH  /content-forms/:id              콘텐츠 폼 수정
DELETE /content-forms/:id              콘텐츠 폼 삭제 (사용 중인 콘텐츠 없을 때만)
```

**Content (10개)**
```
POST   /contents                       콘텐츠 생성
GET    /contents                       콘텐츠 목록 (폼/상태/검색/페이지네이션 필터)
GET    /contents/:id                   콘텐츠 상세
GET    /contents/:contentFormId/slug/:slug  슬러그로 콘텐츠 조회
...
├─ 콘텐츠 관리 ──────────────────────────────┤
│  FormCategory  콘텐츠 폼 카테고리 (name, slug, order)
│  ContentForm   콘텐츠 폼 정의 (fields:JSON, options:JSON, categoryId:FK)
│  Content       콘텐츠 (data:JSONB, status, version, deletedAt, contentFormId:FK)
      ...
      └─ PopupProvider (로딩 스피너, 모달 관리)
          └─ APIProvider (Axios 인스턴스, 인터셉터)
              └─ UserProvider (인증 상태: user, accessToken, refreshToken)
                  └─ GlobalProvider (전역 데이터: contentForms, formCategories, isMobile, sidebarOpen)
                      └─ App
```
...
| **GlobalProvider** | 콘텐츠 폼 목록 캐싱, 모바일 감지(768px), 사이드바 상태          |
...
├── /                            Dashboard           ⬜ 플레이스홀더
├── /login                       로그인              ✅ 완료
├── /form-categories             콘텐츠 폼 카테고리 목록   ✅ 완료
│   ├── /new                     카테고리 생성       ✅ 완료
│   └── /:id/edit                카테고리 수정       ✅ 완료
├── /content-forms               콘텐츠 폼 목록      ✅ 완료
│   ├── /new                     콘텐츠 폼 생성      ✅ 완료
│   └── /:id/edit                콘텐츠 폼 수정      ✅ 완료
├── /contents/:contentFormSlug   콘텐츠 목록         ⬜ 플레이스홀더
├── /media                       미디어 관리         ⬜ 플레이스홀더
...
├── AppHeader (고정 상단)
│   ├── 동적 Breadcrumb (경로 기반 자동 생성, 콘텐츠 폼 slug→이름 변환)
│   └── 햄버거 메뉴 (모바일 전용)
└── AdminMain (flex row)
    ├── AppSidebar (좌측 240px)
    │   ├── 고정 메뉴: 대시보드, 콘텐츠 폼 카테고리, 콘텐츠 폼, 미디어, 역할/권한
    │   ├── 동적 메뉴: "콘텐츠" 그룹 — GlobalContext에서 콘텐츠 폼 목록을 카테고리별 서브그룹으로 렌더링 (독립 토글, order 정렬)
    │   └── 로그아웃 버튼 (하단)
...
2. ~~**콘텐츠 폼 관리 UI**: 목록/생성/수정/삭제 완전 구현~~ ✅ 완료
3. ~~**콘텐츠 폼 카테고리**: FormCategory 엔티티로 콘텐츠 폼 그룹핑~~ ✅ 완료
4. **콘텐츠 관리 UI**: 동적 폼 + TipTap 리치텍스트 에디터
5. **미디어 관리 UI**: 업로드, 폴더 관리, 이미지 미리보기
6. **역할/권한 관리 UI**: 역할 CRUD, 권한 설정, 역할 요청/승인
7. **대시보드**: 전체 현황 통계 (콘텐츠/미디어/사용자 수 등)
...
│  ┌────┴─────────────────────────────────────────────────────┐    │
│  │ Pages                                                     │    │
│  │ Login | Dashboard | FormCategory | ContentForm | Content  │    │
│  │ | Media | Role                                            │    │
│  └───────────────────────────────────────────────────────────┘    │
└──────────────────────────┬───────────────────────────────────────┘
                           │ HTTP (Axios)
...
│                                                                   │
│  ┌─── Modules ─────────────────────────────────────────────────┐ │
│  │ Auth(5)  Role(5)  UserRole(7)  FormCategory(5) ContentForm(5)│ │
...
│  └─────────────────────────────────────────────────────────────┘ │
│                                                                   │
│  Swagger API 문서: /api-docs (9개 태그, 49개 엔드포인트)         │
└───────────┬───────────────────────────┬──────────────────────────┘
            │                           │
     ┌──────┴──────┐            ┌───────┴──────┐
     │ PostgreSQL  │            │    Redis     │
...
[권한 형식]
  ...
  지원 리소스: auth, role, user, content, content-form, form-category, media, task
...
[권한 검사 흐름]
  ...
  PermissionsGuard.canActivate()
    ...
    ├── 콘텐츠 폼 권한: content-form:create/update/delete로 제어 (카테고리 권한도 동일 매핑)
...
[데이터 구조 — JSONB 하이브리드 방식]
  ...
  FormCategory (콘텐츠 폼 카테고리)
  │
  │  name: "블로그"
  │  slug: "blog" (고유)
  │  order: 0 (사이드바 정렬 순서)
  │
  └── ContentForm (콘텐츠 폼 정의)
      │
      │  name: "블로그 포스트"
      │  slug: "blog-post" (URL 라우팅용, 고유)
      │  description: "블로그 게시글 콘텐츠"
      │  categoryId: FK → FormCategory (nullable, 삭제 시 SetNull)
      │  fields: [                              ← JSON 필드 정의
...
[JSONB 방식을 선택한 이유]
  ...
  ❌ 동적 테이블 방식: 콘텐츠 폼마다 DB 테이블 생성 → 마이그레이션 복잡, 관리 어려움
...
[ContentType → ContentForm 리네이밍 참고]

  - model ContentType → ContentForm, @@map("content_forms")
  - API: /content-types → /content-forms, /public/content-types → /public/content-forms
  - 권한: content-type:* → content-form:*
  - 코드 심볼: ContentType/contentType/contentTypeId/contentTypeSlug → ContentForm/contentForm/contentFormId/contentFormSlug
...
| **현재 상태** | 백엔드 API 49개+크론3개 완료, 프론트엔드 기반+콘텐츠 폼 카테고리/콘텐츠 폼 CRUD 완료, 콘텐츠/미디어/역할 UI 구현 중 |
